...
-- Create index on dashboard_share_token for fast lookups
CREATE INDEX IF NOT EXISTS idx_share_profiles_dashboard_token ON public.share_profiles (dashboard_share_token);

-- Drop the function if it exists (to allow recreation with updated signature)
DROP FUNCTION IF EXISTS public.get_dashboard_share_profile_by_token(text);

-- Create the secure function for dashboard token lookup
...
-- Grant execute permissions to authenticated and anonymous roles
