...
import { PlaneTakeoff, PlaneLanding, Users, Filter, Clock, DollarSign, Loader2, AlertCircle, Star, Zap, Heart, Baby, ChevronDown, AlertTriangle, Info, Armchair, CheckCircle2, XCircle, ExternalLink, ArrowUpDown, Bell, TrendingDown, TrendingUp, Minus, Lightbulb, ArrowLeft, ArrowRight } from "lucide-react";
...
const buildGoogleFlightsUrl = (origin: string, destination: string, departDate: string, returnDate?: string, passengers?: number, cabinClass?: string): string => {
...
const Flights = () => {
  ...
  const searchFlights = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('search-flights', {
        body: {
          origin,
          destination,
  ...
  return <AppLayout>
      <div className="container mx-auto px-4 py-6 max-w-5xl">
          ...
          <Card>
            ...
            <CollapsibleContent>
              <CardContent className="space-y-6">
                ...
                {!preferences.prefer_nonstop && <div className="space-y-4">
                    <div>
                      <div className="flex justify-between mb-2">
                        <Label>Max Layover Hours</Label>
                        <span className="text-sm">{preferences.max_layover_hours}h</span>
                      </div>
                      <Slider value={[preferences.max_layover_hours]} onValueChange={([v]) => updatePreferences({
                    max_layover_hours: v
                  })} min={1} max={12} step={1} />
                    </div>
                    <div>
                      ...
                      <Slider value={[preferences.min_connection_minutes]} onValueChange={([v]) => updatePreferences({
                    min_connection_minutes: v
                ...
                <div className="space-y-4 pt-4 border-t">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    label
                        ...
                        <Select value={preferences[key as keyof typeof preferences] as string || 'nice_to_have'} onValueChange={v => updatePreferences({
                      [key]: v
                    })}>
                ...
                <AlternateAirportsSection alternateAirports={preferences.alternate_airports} onUpdate={airports => updatePreferences({
                alternate_airports: airports
              })} />
              </CardContent>
            </CollapsibleContent>
          </Card>
        ...
        <Card className="mb-6">
          <CardContent className="pt-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              value: "multicity",
              label: "Multi-City"
            }].map(type => <Button key={type.value} variant={tripType === type.value ? "default" : "outline"} size="sm" onClick={() => setTripType(type.value as typeof tripType)}>
                  {type.label}
                </Button>)}
            </div>

            {/* Home Airports Quick Select */}
            {preferences.home_airports.length > 0 && <div className="flex flex-wrap gap-2">
            ...
            {tripType === "multicity" && <div className="space-y-4">
                ...
                {multiCitySegments.map((segment, index) => <div key={index} className="p-3 border rounded-lg bg-muted/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <Badge variant="outline">Segment {index + 1}</Badge>
                      {multiCitySegments.length > 2 && <Button variant="ghost" size="sm" onClick={() => {
                  setMultiCitySegments(segments => segments.filter((_, i) => i !== index));
                    ...
                    <div className="grid sm:grid-cols-3 gap-3">
                      <div>
                        ...
                        <Input placeholder="Airport code" value={segment.origin} onChange={e => {
                    ...
                    setMultiCitySegments(newSegments);
                      ...
                      <div>
                        ...
                        <Input placeholder="Airport code" value={segment.destination} onChange={e => {
                    const newSegments = [...multiCitySegments];
                    ...
                    setMultiCitySegments(newSegments);
                      ...
                      <div>
                        ...
                        <Input type="date" value={segment.date} onChange={e => {
                    const newSegments = [...multiCitySegments];
                ...
                {multiCitySegments.length < 6 && <Button variant="outline" size="sm" onClick={() => {
            ...
            {cabinClass === "any" && <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-md">
              ...
              {isPremiumCabin && <div className="p-3 bg-primary/5 rounded-lg border border-primary/20 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {PREMIUM_SEAT_TYPES.map(seatType => <Tooltip key={seatType.value}>
                        <TooltipTrigger asChild>
                          <Badge variant={premiumSeatTypes.includes(seatType.value) ? "default" : "outline"} className="cursor-pointer text-xs" onClick={() => togglePremiumSeatType(seatType.value)}>
                            {seatType.label}
                          </Badge>
                        </TooltipTrigger>
                  ...
                  {premiumSeatTypes.length > 0 && <p className="text-xs text-primary">
                      Looking for: {premiumSeatTypes.map(t => PREMIUM_SEAT_TYPES.find(p => p.value === t)?.label).join(", ")}
                    </p>}
                </div>}
        ...
        {/* Demo Mode Notice */}
        {isDemo && searchMessage && <Card className="mb-6 border-amber-500/50 bg-amber-50 dark:bg-amber-950/20">
        ...
        {/* Category Cards */}
        {categories && <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            {categories.bestOverall && <Card className="border-primary bg-primary/5">
            ...
            {categories.bestForFamilies && preferences.family_mode && <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
        ...
        {/* Search Results */}
        {searchError && <Card className="mb-6 border-destructive">
        ...
        {flights.length > 0 && <div className="space-y-4">
          ...
          return <Card key={flight.id} className={`
                  ...
                  <CardContent className="p-4">
                    ...
                    {isAvoided && <div className="flex items-center gap-2 mb-2 p-2 bg-red-100 dark:bg-red-950/40 rounded-md text-red-700 dark:text-red-400">
                        <AlertTriangle className="h-4 w-4" />
                        <span className="text-sm font-medium">Avoided Airline</span>
                      </div>}

                    {/* Alternate Airport Badge */}
                    {flight.isAlternateOrigin && flight.departureAirport && <div className="flex flex-wrap items-center gap-2 mb-2 p-2 bg-orange-50 dark:bg-orange-950/30 rounded-md">
                        <Badge className="bg-orange-500 hover:bg-orange-600 text-white">
                          From {flight.departureAirport}
                        </Badge>
                        {flight.savingsFromPrimary && flight.savingsFromPrimary > 0 && <div className="flex flex-wrap gap-2 text-sm text-orange-700 dark:text-orange-400 font-medium">
                            <span>Save ${flight.savingsFromPrimary.toFixed(0)} total</span>
                            {passengers > 1 && flight.savingsPerTicket && <span className="text-orange-600 dark:text-orange-300">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {flight.badges.map(b => <Badge key={b} variant="secondary" className="text-xs">{b}</Badge>)}
                          {flight.delayRisk === "high" && <Badge variant="destructive" className="text-xs">
                      ...
                      <div className="text-right">
                        ...
                        {passengers > 1 && flight.pricePerTicket && <p className="text-sm text-muted-foreground">
                            ${flight.pricePerTicket.toFixed(0)}/ticket
                          </p>}
                        {flight.hiddenCosts.length > 0 && <p className="text-xs text-muted-foreground">
                            +${flight.hiddenCosts.reduce((s, c) => s + c.estimatedCost, 0)} est. fees
                          </p>}
                        {/* Price Insight Badge */}
                        {flight.priceInsight && <Badge variant="outline" className={`mt-1 text-xs ${flight.priceInsight.level === "low" ? "border-green-500 bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400" : flight.priceInsight.level === "high" ? "border-red-500 bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400" : "border-yellow-500 bg-yellow-50 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-400"}`}>
                    ...
                    {flight.priceInsight && <div className={`text-xs p-2 rounded-md mb-3 flex items-start gap-2 ${flight.priceInsight.level === "low" ? "bg-green-50 dark:bg-green-950/20 text-green-700 dark:text-green-400" : flight.priceInsight.level === "high" ? "bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-400" : "bg-yellow-50 dark:bg-yellow-950/20 text-yellow-700 dark:text-yellow-400"}`}>
                    ...
                    {idx === 0 && sortBy === "score" && flight.score < 100 && flight.matchExplanation && <div className="mb-3 p-3 bg-muted/50 rounded-lg space-y-2">
                        <div className="flex items-center gap-1.5 text-sm font-medium">
                          <Lightbulb className="h-4 w-4 text-amber-500" />
                          <span>Why this is your best match:</span>
                        </div>
                        
                        {flight.matchExplanation.whyNotPerfect.length > 0 && <div className="text-xs space-y-1">
                            <p className="text-muted-foreground font-medium">Not a 100% match because:</p>
                            <ul className="list-disc list-inside text-muted-foreground space-y-0.5">
                              {flight.matchExplanation.whyNotPerfect.map((reason, i) => <li key={i}>{reason}</li>)}
                            </ul>
                          </div>}
                        
                        {flight.matchExplanation.whyBestChoice.length > 0 && <div className="text-xs space-y-1">
                            ...
                            <ul className="list-disc list-inside text-green-700 dark:text-green-400 space-y-0.5">
                              {flight.matchExplanation.whyBestChoice.map((reason, i) => <li key={i}>{reason}</li>)}
                            </ul>
                          </div>}
                      </div>}

                    {/* For non-first results, show why it's not the best match */}
                    {idx > 0 && sortBy === "score" && flight.matchExplanation && flight.matchExplanation.whyNotPerfect.length > 0 && <div className="mb-3 p-2 bg-muted/30 rounded-md">
                    ...
                    {flight.preferenceMatches && flight.preferenceMatches.length > 0 && <div className="flex flex-wrap gap-2 mb-3">
                        {flight.preferenceMatches.filter(m => m.type === "positive").map((match, i) => <Tooltip key={`pos-${i}`}>
                            <TooltipTrigger asChild>
                            ...
                            {match.detail && <TooltipContent>
                                <p>{match.detail}</p>
                              </TooltipContent>}
                          </Tooltip>)}
                        {flight.preferenceMatches.filter(m => m.type === "negative").map((match, i) => <Tooltip key={`neg-${i}`}>
                            <TooltipTrigger asChild>
                ...
                return <div key={itIdx} className="space-y-2 mb-3">
                          {/* Itinerary header */}
                          <div className={`flex items-center gap-2 ${itIdx > 0 ? 'border-t pt-3' : ''}`}>
                            ...
                            {itinerary.segments?.length > 1 && <span className="text-xs text-muted-foreground">
                    ...
                    return <div key={segIdx} className="space-y-2">
                          ...
                          const googleFlightsUrl = buildGoogleFlightsUrl(seg.departureAirport, seg.arrivalAirport, seg.departureTime ? new Date(seg.departureTime).toISOString().split('T')[0] : departDate, tripType === "roundtrip" ? returnDate : undefined, passengers, cabinClass);
                          window.open(googleFlightsUrl, '_blank');
                        }} className="gap-1" title="Book on Google Flights - final price confirmed there">
                                    Book on Google <ExternalLink className="h-3 w-3" />
                                  </Button>
                                </div>
                                
                                {/* Connection Risk Indicator - show between segments */}
                                {hasLayover && layover && <div className="flex items-center gap-2 pl-4 py-1 border-l-2 border-dashed border-muted ml-4">
                                    <Clock className="h-3 w-3 text-muted-foreground" />
                                    <span className="text-xs text-muted-foreground">
                                      {layover.duration}min layover at {layover.airport || layover.airportName}
                                    </span>
                                    <ConnectionRiskIndicator layoverMinutes={layover.duration} hasKids={preferences.family_mode || children > 0 || infantsInSeat > 0} isInternational={origin.length === 3 && destination.length === 3} />
                                  </div>}
                              </div>;
                  })}
                        </div>;
                    ...
                    {tripType === "roundtrip" && flight.itineraries?.length === 1 && <div className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-800 mb-3">
                    ...
                    <FlightTransparencyPanel cabinClass={cabinClass === "any" ? "economy" : cabinClass} isBasicEconomy={flight.badges?.includes("Basic Economy")} fareFamily={flight.itineraries[0]?.segments[0]?.cabin} inclusions={{
              ...
              }} restrictions={flight.badges?.includes("Basic Economy") ? ["No carry-on bag (personal item only)", "Seat assigned at check-in", "Board last", "No changes or refunds"] : []} estimatedBagFees={{
                ...
                secondBag: cabinClass === "business" || cabinClass === "first" ? 0 : 45
                    ...
                    {tripType === "roundtrip" && <Button className="w-full mt-3 gap-2" onClick={() => {
                const firstSeg = flight.itineraries[0]?.segments[0];
                const googleFlightsUrl = buildGoogleFlightsUrl(firstSeg?.departureAirport || origin, firstSeg?.arrivalAirport || destination, departDate, returnDate, passengers, cabinClass);
                window.open(googleFlightsUrl, '_blank');
              }}>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-2">
                      {tripType === "roundtrip" ? "Price shown is for round-trip. Final price and return flight options confirmed on Google Flights." : "Estimated fare shown. Final price and inclusions confirmed on Google Flights/airline site."}
                    </p>
                  </CardContent>
                </Card>;
        })}
        ...
        {/* Price Alert Dialog */}
        <PriceAlertDialog open={priceAlertOpen} onOpenChange={setPriceAlertOpen} origin={origin} destination={destination} departureDate={departDate} returnDate={tripType === "roundtrip" ? returnDate : undefined} currentPrice={flights[0]?.price || 0} passengers={passengers} />
      </div>
    </AppLayout>;
};
