...
const Flights = () => {
  ...
  useEffect(() => {
    if (tripType === "roundtrip" && outboundSelected && isReturnPending) {
      // Extract primary airline from outbound flight for consistency checking
      const outboundAirline = outboundSelected.flight?.itineraries[0]?.segments[0]?.airline;
      searchReturnLeg(outboundAirline);
    }
  ...
  const searchFlights = async () => {
    if (tripType === "multicity") {
      ...
      // Validate multi-city dates are in chronological order
      for (let i = 1; i < validSegments.length; i++) {
        const prevDate = new Date(validSegments[i - 1].date);
        const currDate = new Date(validSegments[i].date);
        if (currDate < prevDate) {
          toast.error(`Flight ${i + 1} date must be on or after Flight ${i} date`);
          return;
        }
      }
      setSelectedFlights([]);
